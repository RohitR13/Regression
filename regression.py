...
def random_fourier_features(train_X, train_Y, num_fourier_features=100, alpha=0.1, num_iters=500, print_iters=False):
	...
	Omega = numpy.random.multivariate_normal(numpy.zeros(num_fourier_features), numpy.eye(num_fourier_features), size = train_X.shape[1])	# TODO: sample inner-products
	B = 2 * math.pi * numpy.random.random(size = num_fourier_features)		# TODO: sample translations
...
def rff_model_sample(Theta,Omega,B,model_X):
	sampled_X = numpy.linspace(model_X.min(axis=0),model_X.max(axis=0), 1100)
